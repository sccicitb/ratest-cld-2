...
- [ ] **Task 2.1 — Embedder.** Test: `embed_query("hello")` returns dense len 1024 + non-empty sparse; `embed_passages` batches. Implement the **FlagEmbedding** wrapper + process-singleton: `BGEM3FlagModel("BAAI/bge-m3", use_fp16=True)`, call `model.encode(texts, return_dense=True, return_sparse=True)`; map each result to `Embedding{dense: list[float], sparse: {indices, values}}` (BGE-M3's `lexical_weights` → Qdrant sparse `indices`/`values`).
