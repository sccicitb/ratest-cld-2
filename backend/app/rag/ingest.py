...
async def ingest(
    ...
    Yields `chunk_progress` events as batches are embedded/upserted. On
    extraction/chunking failure, marks the file `status="error"` and
    re-raises so the caller (route) can surface an error event.
    """
